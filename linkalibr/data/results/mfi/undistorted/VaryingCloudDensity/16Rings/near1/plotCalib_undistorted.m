clear all;
clc;
data_ext_calib = csvread('lin_calib_extrinsics_near1_undistortedcloud.csv');
quat = data_ext_calib(:, 1:4);
xyz = data_ext_calib(:, 5:7);
sigma_xyz = data_ext_calib(:, 11:13);
sigma_rxryrz = data_ext_calib(:, 8:10)*180/pi;
data_dt_calib = csvread('lin_calib_dt_near1_undistortedcloud.csv');
dt = data_dt_calib(:, 1);
...
%hold off;
hold on;
plot(dt(:,1) + sigma_dt(:,1), '--r', 'LineWidth', 3);
hold on;
plot(dt(:,1) - sigma_dt(:,1), '--r', 'LineWidth', 3);
hold off;
ylabel('time-offset','fontweight','bold','fontsize',16);
grid;
title('Time offset vs time plot','fontweight','bold','fontsize',16);
...
%%
figure('Name','Time Delay Calibration KF','NumberTitle','off');
time_delay = dt(:,1)*1000;
plot(time_delay, 'LineWidth', 3);
ylabel('t_d [ms]','fontweight','bold','fontsize',30);
grid;
title('Difference between IMU Lidar temporal latencies (t_d) vs time plot','fontweight','bold','fontsize',30);
ylim([min(time_delay)*2, max(time_delay)*2]);
